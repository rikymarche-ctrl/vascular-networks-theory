...
(dimensionless), analogous to geometric frustration in physical systems---\chE{suggests that optimization is displaced to a higher topological level}. By grounding the fitness
...
equilibrium). \chE{This dual state is not naturally predicted by purely local optimization}
...
characteristic wave admittance \chA{$Y_c \propto \sqrt{i\omega\rho\,Y_L}$, equivalently $Y_c\propto\sqrt{1-F_{10}}$}---yielding two separate
...
\chA{purely deductive result}. This ansatz provides a sufficient
condition for the transition threshold. The exact Bessel solution, which fully
accounts for all modal coupling, \chA{supports} the threshold to within $0.5\%$,
