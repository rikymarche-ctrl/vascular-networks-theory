...
such extreme fine-tuning is biologically implausible, \chE{real networks appear
to require symmetry-breaking features} (e.g., via taper or asymmetric branching)
to achieve scalable local optimization.
\end{proposition}

\chRB{Intuitively, the proposition says that a vessel acting on local
information alone cannot keep the \emph{entire} tree scale-free. Picture a
bricklayer asked to build a self-similar pattern that must look the same whether
the wall ends up one metre or one kilometre tall, but who can see only the brick
in hand and its immediate neighbours. To place each brick correctly the
bricklayer would need to know how far up the wall it sits---a global quantity.
The vascular counterpart of ``height up the wall'' is the generation depth $g$,
and the only way to infer it from local cues is through the absolute vessel
size, which in turn requires knowing the size of the root (aortic) vessel. As
Step~3 of the proof \chA{shows}, that root scale is precisely the piece of
information a downstream junction cannot recover from local measurements---which
is why \chE{a strictly local, symmetric rule is not sufficient under these
assumptions}.}
...
$\mathcal{C}_{\mathrm{visc}}\propto r^{-4}$, while the $\mathcal{O}(r^{-3})$
wave gradient integrates to $\mathcal{C}_{\mathrm{wave}}\propto r^{-2}$; their
ratio therefore grows as $\Lambda(r) =
\mathcal{C}_{\mathrm{wave}}/\mathcal{C}_{\mathrm{visc}} \propto r^{-2}/r^{-4} =
r^{2}$, so a vessel ten times wider sees the wave penalty inflated a hundredfold
relative to the viscous one.}
...
This computation \chE{requires, within this formulation, access to $r_0$} (the
root absolute scale). While downstream reflections encode the local input
impedance, the physical information regarding the upstream root scale $r_0$ is
topologically shielded and directionally inaccessible to a local junction. Like
a transmission line terminated by a complex load, the local impedance
measurement cannot uniquely determine the source characteristics.
...
precise cycle-by-cycle phase offset, the \chE{local cellular machinery is
unlikely to resolve} the wave transport parameters necessary for impedance-based
tuning.
...
maintaining a constant shear stress target $\tau_w \approx c$) are \chE{expected
to converge to Murray's local attractor in standard shear-homeostatic models}
($\alpha = 3$). However, in large conduit arteries, maintaining $\alpha=3$ would
cause \chE{substantial wave-reflection costs}, severely compromising cardiac
efficiency. The transition to $\alpha \approx 2.7$ in the large vessels of large
mammals requires the cell to actively break the local Murray feedback loop.
Because a cell cannot determine when to suppress this feedback without knowing
its global position $g$ or the species-specific body mass $M$, the transition
rule cannot be encoded locally, \chE{supporting a network-level minimax
interpretation}.
\end{enumerate}
...
fragile, leaving the global network-level minimax attractor as \chE{a robust,
information-economical mechanism} for scale-invariant transport.
\end{remark}
...
non-locality, not a Shannon channel-capacity bound, that \chE{rules out purely
local optimization within the stated assumptions}.}
\end{proof}
