...
such extreme fine-tuning is biologically implausible, real networks appear
to require symmetry-breaking features (e.g., via taper or asymmetric branching)
to achieve scalable local optimization.
\end{proposition}

Intuitively, the proposition says that a vessel acting on local
...
Step~3 of the proof shows, that root scale is precisely the piece of
information a downstream junction cannot recover from local measurements---which
is why a strictly local, symmetric rule is not sufficient under these
assumptions.
...
$g$. This $r^{2}$ growth is a direct consequence of Step~(1): a viscous
...
This computation requires, within this formulation, access to $r_0$ (the
...
maintaining a constant shear stress target $\tau_w \approx c$) are expected
...
cause substantial wave-reflection costs, severely compromising cardiac
...
rule cannot be encoded locally, supporting a network-level minimax
interpretation.
\end{enumerate}
...
fragile, leaving the global network-level minimax attractor as a robust,
...
vasculature, supporting the need for network-level optimization.
\end{corollary}

\begin{corollary}[Information-Accounting Argument]
\label{cor:information}
Setting a generation-dependent profile that holds $\alpha^*$ constant
...
\begin{proof}
To regulate the wall profile $\mu(g)$ a junction need not resolve its full
...
non-locality, not a Shannon channel-capacity bound, that rules out purely
