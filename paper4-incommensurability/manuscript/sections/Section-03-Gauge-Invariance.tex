...
fractional excess. The linear penalty is therefore not merely a modeling
...
It is worth pausing on an apparent tension between the two statements just
...
arbitrary mathematical choice, but a direct consequence within the stated
...
$0.3\%$. These alternative penalties are merely counterfactual numerical
robustness tests, less admissible under the four criteria used here. Their
...
fundamental physical lower bound ($\Phi \to \Phi^*$). In this near-optimal
...
$\Phi(\vec{x})$ subject to morphometric constraints (fixed total volume and
...
\begin{equation}
    ...
    + O\left(\|\delta\vec{x}\|^3\right)
\end{equation}
where $\lambda_{\mathrm{bio}} > 0$ is a phenotype-specific selection
coefficient, and the penalty is linear in the excess dissipation
...
This theorem is the engine behind the linear cost functional used
...
\begin{proof}[Proof sketch; see Supplemental Material, Section S7 for full derivation]
...
\lesssim 0.1$, supporting operation in the linear response regime.
...
results, though alternative functionals are numerically near-degenerate:
...
In words, this equation demands that scoring the combined network directly
...
This is the \emph{weighted Jensen (affinity) equation}: the condition
...
Metabolic scale invariance---requiring the cost functional to be unchanged
...
species-specific deviations from $M^{3/4}$ scaling; current evidence shows
no such structure.
\end{remark}
...
$\beta_M(\alpha,d) = d\alpha/(2d+\alpha)$ (the organismal metabolic
...
scale $\Phi_{\mathrm{opt}}$ as the metabolic normalization—analogous to
...
of Paper~II~\cite{paperII} supports the saddle point $\alpha^* =
\VarAlphaStar$ is stable across all convex aggregations $q \geq 1$, with a total
spread $\Delta\alpha^* = 0.008$. A systematic test of the most natural
...
Onsager linearity, and non-substitutability, supporting the conclusion
that the linear excess is the admissible choice under the stated
...
(Table~\ref{tab:sensitivity}), showing that the architecture is decoupled
...
Table~\ref{tab:sensitivity} shows the two-regime structure: pure metabolic
...
across all mammalian species, supporting the robustness of the minimax
attractor.
\end{remark}
