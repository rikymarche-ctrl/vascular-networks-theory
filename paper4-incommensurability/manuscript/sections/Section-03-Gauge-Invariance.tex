...
fractional excess. \chE{The linear penalty is therefore not merely a modeling
choice under these assumptions}, but a consequence grounded in ATP stoichiometry
and constrained by the first law of thermodynamics.

\chRB{It is worth pausing on an apparent tension between the two statements just
made---that the cost is proportional to the \emph{absolute} energetic excess,
yet must take the form of a \emph{fractional} excess. The two are reconciled by
separating \emph{within}-organism from \emph{across}-organism comparisons. At a
fixed body size the penalty is linear in the absolute excess power, because ATP
is charged watt by watt; but across body sizes spanning five orders of magnitude
in absolute metabolism, selection can act only on the \emph{dimensionless}
excess, so the absolute power must be normalized by the organism's own optimum
$\Phi_{\mathrm{opt}}$. The fractional excess
$(\Phi-\Phi_{\mathrm{opt}})/\Phi_{\mathrm{opt}}$ is the unique form that is
simultaneously linear in the wasted power \emph{and} carries the same meaning
for a shrew and a whale.}
...
arbitrary mathematical choice, but a \chE{direct consequence within the stated
thermodynamic approximation} of oxidative phosphorylation stoichiometry and
near-equilibrium thermodynamics. At the cellular level, excess fluidic
dissipation is paid in units of ATP. Because this metabolic accounting is
strictly additive---one extra Joule of dissipated physical work requires a
strictly proportional increase in oxidative metabolism---the evolutionary
fitness penalty must scale linearly with excess entropy production. Imposing
non-linear penalties (e.g., quadratic or logarithmic) would imply unphysical
non-stoichiometric metabolic costs, equivalent to postulating ``compound
interest'' on ATP molecules.

Furthermore, numerical sensitivity analysis \chA{shows} that imposing artificial
non-linear penalties alters the theoretical attractor $\alpha^*$ by less than
$0.3\%$. \chE{These} alternative penalties are merely counterfactual numerical
robustness tests, \chE{less admissible under the four criteria used here}. Their
negligible impact is the signature of Prigogine's minimum entropy production
principle in the Onsager regime: because the evolutionary attractor is strongly
confining, the mature vascular network operates in the immediate vicinity of the
fundamental physical lower bound ($\Phi \to \Phi^*$). \chA{In this near-optimal
regime, the fitness penalty is governed by the excess dissipation
$\Delta\Phi=\Phi-\Phi^*$, in which it is linear; since the optimum is a
stationary point ($\nabla\Phi(\vec{x}^*)=0$), $\Delta\Phi$ is in turn quadratic
in the configurational deviation $\delta\vec{x}$.}
...
$\Phi(\vec{x})$ subject to morphometric constraints (fixed total volume \chE{and
...
\begin{equation}
    ...
    \chA{+ O\left(\|\delta\vec{x}\|^3\right)}
\end{equation}
where $\lambda_{\mathrm{bio}} > 0$ is a phenotype-specific selection
coefficient\chA{, and the penalty is linear in the excess dissipation
$\Delta\Phi=\Phi-\Phi^*$; near the optimum $\Delta\Phi$ is itself
$O(\|\delta\vec{x}\|^2)$, since $\nabla\Phi(\vec{x}^*)=0$ (Step~1)}.
...
\chRB{This theorem is the engine behind the linear cost functional used
throughout the paper, so it is worth stating its content in words. It makes two
linked claims. First, near the optimal architecture the evolutionary penalty
grows in proportion to the \emph{fractional} energy waste
$\Delta\Phi/\Phi_{\mathrm{opt}}$---not its square, nor its logarithm---because
metabolism charges a fixed number of ATP molecules per excess watt, with no
``compound interest.'' Second, that fractional waste is itself only a
\emph{second-order} (quadratic) function of how far the vessel geometry strays
from optimal, because at a minimum the first derivative of the dissipation
vanishes. Together these mean that a small geometric error incurs only a gentle,
second-order metabolic penalty---which is exactly why real networks can tolerate
the modest morphometric heterogeneity observed in practice while still sitting
very close to the optimum.}

\begin{proof}[Proof sketch; see Supplemental Material, \chA{Section S7} for full derivation]
...
\lesssim 0.1$, \chA{supporting} operation in the linear response regime.
Explicit calculation for the human vasculature at $\alpha = 2.39$ yields:
\begin{equation}
...
results, though \chE{alternative} functionals are numerically near-degenerate:
the logarithmic penalty shifts $\alpha^*$ by $\Delta \alpha^* \approx -0.004$,
while the quadratic case yields $\Delta \alpha^* \approx +0.007$.
...
\chRB{In words, this equation demands that scoring the combined network directly
(left-hand side) yield exactly the same number as averaging the two separate
subsystem scores (right-hand side). A functional that failed it would rank a
given vasculature as more or less wasteful depending only on where one happened
to draw the boundary between subsystems, which is physically meaningless.}

This is \chA{the \emph{weighted Jensen (affinity) equation}: the condition
obtained when Jensen's inequality is required to hold with equality for all
weights $w_A,w_B$, not the classical midpoint Jensen equation
$F\!\left(\tfrac{x+y}{2}\right)=\tfrac{F(x)+F(y)}{2}$.} Assuming $F$ is
differentiable, differentiating both sides with respect to $x_A$ yields:
\begin{equation}
...
the fractional excess dissipation. This linearity is \chA{supported} by three
...
\chA{Metabolic scale invariance}---requiring the cost functional to be unchanged
...
species-specific deviations from $M^{3/4}$ scaling; current evidence \chA{shows}
no such structure.
\end{remark}
...
\chA{$\beta_M(\alpha,d) = d\alpha/(2d+\alpha)$ (the organismal metabolic
exponent, not to be confused with the branching ratio $\beta$)} from proximal
dominance (Theorem~1), showing that scale invariance uniquely maps local
branching geometry to global organismal scaling laws.
\end{remark}
...
scale $\Phi_{\mathrm{opt}}$ as the \chA{metabolic normalization}—analogous to
fixing the zero of the electrostatic potential in classical electrodynamics, not
to local gauge symmetries (U(1), SU(2)) of field theory. The invariance
...
of Paper~II~\cite{paperII} \chA{supports} the saddle point $\alpha^* =
\VarAlphaStar$ is stable across all convex aggregations $q \geq 1$, with a total
spread $\Delta\alpha^* = 0.008$. \chRB{A systematic test of the most natural
alternative functional \emph{forms}---quadratic, logarithmic, power-law, and
absolute-threshold---against scale invariance, compositional consistency,
Onsager linearity, and non-substitutability, \chA{supporting} the conclusion
that the linear excess is the \chA{admissible choice under the stated
assumptions}, is provided in the Supplemental Material (Section~S7).}
\end{remark}
...
(Table~\ref{tab:sensitivity}), \chA{showing} that the architecture is decoupled
...
Table~\ref{tab:sensitivity} \chA{shows} the two-regime structure: pure metabolic
...
across all mammalian species, \chA{supporting} the robustness of the minimax
attractor.
\end{remark}
