...
fractional excess. \chE{The linear penalty is therefore not merely a modeling choice under these assumptions}, but a
...
made---that the cost is proportional to the \emph{absolute} energetic excess, yet
must take the form of a \emph{fractional} excess. The two are reconciled by
...
simultaneously linear in the wasted power \emph{and} carries the same meaning for
a shrew and a whale.}
...
arbitrary mathematical choice, but a \chE{direct consequence within the stated thermodynamic approximation} of oxidative
...
$0.3\%$. \chE{These} alternative penalties are merely
counterfactual numerical robustness tests, \chE{less admissible under the four criteria used here}. Their negligible impact is the signature of Prigogine's minimum
...
\chA{In this near-optimal regime, the fitness penalty is governed by the excess
dissipation $\Delta\Phi=\Phi-\Phi^*$, in which it is linear; since the optimum is
...
\chE{and hierarchical connectivity}).
...
\lesssim 0.1$, \chA{supporting} operation in the linear response regime. Explicit
...
results, though \chE{alternative} functionals are numerically
...
$F\!\left(\tfrac{x+y}{2}\right)=\tfrac{F(x)+F(y)}{2}$.} Assuming $F$ is differentiable,
...
species-specific deviations from $M^{3/4}$ scaling; current evidence \chA{shows} no
such structure.
\end{remark}
...
exponent, not to be confused with the branching ratio $\beta$)} from proximal dominance (Theorem~1),
...
scale $\Phi_{\mathrm{opt}}$ as the \chA{metabolic normalization}—analogous to fixing the
...
Onsager linearity, and non-substitutability, \chA{supporting} the conclusion that the linear excess
is the \chA{admissible choice under the stated assumptions}, is provided in the Supplemental Material
(Section~S7).}
\end{remark}
