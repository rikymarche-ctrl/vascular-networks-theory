...
\section{Architectural Invariance of the \texorpdfstring{\chA{Marginal-Balance Weight}}{Marginal-Balance Weight}}
\label{sec:arch}
...
\begin{theorem}[Architectural Invariance of the \chA{Marginal-Balance Weight}]
\label{thm:arch}
...
\end{equation}
The emergent \chA{marginal-balance weight} $\eta^*$ is an exact invariant of the
...
\chA{Because $\mathcal{L}_{\mathrm{net}}$ is linear in $\eta$, the worst-case
...
(Supplemental Material, Table S1) \chA{shows} that the \chA{scalar} Hessian
$\partial^2 \mathcal{C}_{\mathrm{wave}}/\partial\alpha^2 > 0$ throughout the
physiological range $\alpha \in [2.0, 3.5]$ and $\eta \in [0,1]$, \chA{securing
...
\begin{equation}
    ...
    \label{eq:eta_star}
\end{equation}
\chA{Because $\mathcal{L}_{\mathrm{net}}$ is affine in $\eta$, an interior
...
\end{equation*}
which is the equal-cost condition.} \chRB{This is the formal counterpart of the
...
emergent \chA{marginal-balance weight} $\eta^*$ effectively balances the two
...
The \chA{marginal-balance weight} $\eta^*$ is therefore a pure structural
invariant, determined solely by $(G, p, \alpha_w)$ through the geometric
evaluation of both derivatives at $\alpha^*$.
\end{proof}

\begin{remark}
This provides a \chA{mechanistic explanation} for the empirical observation that
...
\chA{supporting the minimax solution as the relevant physical attractor} within
these constraints.
\end{remark}

\begin{remark}[Convexity Verification: Physical and Numerical]
\label{rem:quasiconvex}
\chA{Existence and uniqueness of the saddle in Theorem~\ref{thm:arch} require
...
$\alpha \in [2.0, 3.5]$ and $\eta \in [0,1]$ \chA{supports} strict unimodality
...
without \chA{fitting any parameter to the target morphometric exponent}
...
$\alpha_{\exp} = 2.70 \pm 0.20$, \chA{showing} that the network-level Lagrangian
...
\approx \VarAlphaStar$ is a \chA{structural residual} that requires the
