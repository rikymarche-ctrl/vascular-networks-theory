...
convex and concave) in $\eta$ and quasi-convex in $\alpha$ for every fixed $\eta
\in [0,1]$: $C_{\mathrm{transport}}$ is strictly convex, while
$C_{\mathrm{wave}}$ is quasi-convex with a unique minimum at $\alpha_w$. The
domains $[\alpha_w, \alpha_t]$ and $[0,1]$ are compact, and
$\mathcal{L}_{\mathrm{net}}$ is continuous. By Sion's minimax
theorem~\cite{sion1958}, which requires only quasi-convexity, a saddle point
exists. Detailed numerical verification (Supplemental Material, Table S1)
confirms that the Hessian $\partial^2
\mathcal{C}_{\mathrm{wave}}/\partial\alpha^2 > 0$ throughout the physiological
range $\alpha \in [2.0, 3.5]$ and $\eta \in [0,1]$, satisfying Sion's condition
for existence and uniqueness of the saddle point.
...
\begin{equation}
    ...
    \label{eq:eta_star}
\end{equation}
By the envelope theorem, the maximization over $\eta$ in the saddle-point
problem requires $\partial\mathcal{L}_{\mathrm{net}}/\partial\eta = 0$ at the
optimum, which directly implies the equal-cost condition
$\mathcal{C}_{\mathrm{wave}}^{\mathrm{net}}(\alpha^*) =
\mathcal{C}_{\mathrm{transport}}^{\mathrm{net}}(\alpha^*)$. This equivalence
ensures that the emergent duty cycle $\eta^*$ effectively balances the two
...
The duty cycle $\eta^*$ is therefore a pure structural invariant, determined
solely by $(G, p, \alpha_w)$ through the geometric evaluation of both
...
\begin{remark}[Quasi-Convexity Verification: Physical and Numerical]
\label{rem:quasiconvex}
The application of Sion's minimax theorem requires that
$\mathcal{C}_{\mathrm{wave}}(\alpha)$ be quasi-convex in $\alpha$ for each fixed
$\eta$. While not strictly convex, quasi-convexity (unimodal with unique
minimum) is guaranteed by the following physical and numerical arguments:
...
power-law form in viscous dissipation). Therefore, the Lagrangian
$\mathcal{L}_{\mathrm{net}}(\alpha,\eta)$ satisfies the quasi-convexity
requirement of Sion's theorem, ensuring existence of the saddle point. The
