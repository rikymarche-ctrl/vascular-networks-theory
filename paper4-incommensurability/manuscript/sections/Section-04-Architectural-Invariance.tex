...
\section{Architectural Invariance of the \texorpdfstring{Marginal-Balance Weight}{Marginal-Balance Weight}}
\label{sec:arch}
...
It helps to read this construction as a game. The network chooses the
...
waves can be punished by an adversary who emphasizes the wave channel, and a
tree tuned to suppress reflections can be punished by one who emphasizes
transport. The network's safest move is therefore the geometry at which the
adversary's choice no longer matters---where the two costs are \emph{exactly
equal}, so tilting $\eta$ in either direction changes nothing. That equal-cost
geometry is the saddle point $(\alpha^*,\eta^*)$, and its defining property is
\emph{immunity} to the operating weight: the architecture is optimized not for
any single regime but against the worst case across all of them. This is the
precise sense in which $\alpha^*$ is robust rather than fine-tuned.

\begin{theorem}[Architectural Invariance of the Marginal-Balance Weight]
\label{thm:arch}
...
\end{equation}
The emergent marginal-balance weight $\eta^*$ is an exact invariant of the
network's allometric class $\mathcal{A}(G, p, \alpha_w)$, strictly orthogonal to
...
convex and concave) in $\eta$ and convex in $\alpha$ for every fixed $\eta
\in [0,1]$: $C_{\mathrm{transport}}$ is strictly convex, while
$C_{\mathrm{wave}}$ has $\partial^2 C_{\mathrm{wave}}/\partial\alpha^2 > 0$
throughout the physiological range (verified below), hence is convex there,
with a unique minimum at $\alpha_w$. The domains $[\alpha_w, \alpha_t]$ and
$[0,1]$ are compact, and $\mathcal{L}_{\mathrm{net}}$ is continuous.
Because $\mathcal{L}_{\mathrm{net}}$ is linear in $\eta$, the worst-case
cost is attained at an endpoint,
\begin{equation*}
...
$[\alpha_w,\alpha_t]$, and the two costs cross exactly once in the interior,
this minimiser is unique and satisfies the equal-cost condition
...
quasiconvex--quasiconcave minimax theorem. Detailed numerical verification
(Supplemental Material, Table S1) shows that the scalar Hessian
$\partial^2 \mathcal{C}_{\mathrm{wave}}/\partial\alpha^2 > 0$ throughout the
physiological range $\alpha \in [2.0, 3.5]$ and $\eta \in [0,1]$, securing
the convexity on which this argument rests.
...
\begin{equation}
    ...
    \label{eq:eta_star}
\end{equation}
Because $\mathcal{L}_{\mathrm{net}}$ is affine in $\eta$, an interior
saddle with $\eta^*\in(0,1)$ can occur only when the maximiser is indifferent
between the two endpoint costs; equivalently,
\begin{equation*}
...
\end{equation*}
which is the equal-cost condition. This is the formal counterpart of the
game intuition above: at the saddle the adversary is indifferent to $\eta$
precisely because the two costs coincide. This equivalence ensures that the
emergent marginal-balance weight $\eta^*$ effectively balances the two
incommensurable penalties. The numerator derives from the acoustic impedance
...
The marginal-balance weight $\eta^*$ is therefore a pure structural
invariant, determined solely by $(G, p, \alpha_w)$ through the geometric
evaluation of both derivatives at $\alpha^*$.
\end{proof}

\begin{remark}
This provides a mechanistic explanation for the empirical observation that
...
supporting the minimax solution as the relevant physical attractor within
these constraints.
\end{remark}

\begin{remark}[Convexity Verification: Physical and Numerical]
\label{rem:quasiconvex}
Existence and uniqueness of the saddle in Theorem~\ref{thm:arch} require
only that $\mathcal{C}_{\mathrm{wave}}(\alpha)$ be convex in $\alpha$ for each
fixed $\eta$. We establish this through the following physical and numerical
arguments:
...
$\alpha \in [2.0, 3.5]$ and $\eta \in [0,1]$ supports strict unimodality
...
power-law form in viscous dissipation). Since $\mathcal{C}_{\mathrm{wave}}$
...
point. The minimax equal-cost condition $f(\alpha^*) = g(\alpha^*)$ is verified
without fitting any parameter to the target morphometric exponent
(Paper~III~\cite{paperIII}): for the porcine coronary tree, solving this balance
yields $\alpha^* = 2.77$, consistent with the morphometric average
$\alpha_{\exp} = 2.70 \pm 0.20$, showing that the network-level Lagrangian
balances measurable first-principles costs.
\end{remark}
...
\approx \VarAlphaStar$ is a structural residual that requires the
inclusion of the dynamic, pulsatile minimax mechanism developed here.
\end{remark}
