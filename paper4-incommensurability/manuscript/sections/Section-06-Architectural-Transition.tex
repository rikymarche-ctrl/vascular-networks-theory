...
\chA{physically constrained derivation}, the wave penalty $\mathcal{C}_{\mathrm{wave}}$ is
...
\chA{physical matching condition}---combining Euclidean geometry, classical kinematics, and
...
\chA{physical foundation} for the allometric transition.
...
wave} (governed by the characteristic admittance $Y_c$) which \chA{governs} network
reflections. Because \chA{$Y_c \propto \sqrt{i\omega\rho\,Y_L}$, equivalently $Y_c\propto\sqrt{1-F_{10}}$}, the wave phase is algebraically
...
algebraic phase-halving \chA{$Y_c\propto\sqrt{i\omega\rho\,Y_L}$, equivalently $Y_c\propto\sqrt{1-F_{10}}$} as its physical basis. We adopt
...
\chA{connecting the phenomenological observations with the physical}
theory.

\chA{For clarity, the \emph{quantitative} predictions of this
...
toward the wave-attractor limit ($\alpha \approx 2.0$), \chA{supporting the interpretation} that the
...
scaling of Eq.~\eqref{eq:wo_scaling}. \textbf{\chE{At present:}} To our knowledge, no
...
channels are physically active} in a given network. This yields a \chRB{pre-specified},
...
datasets provide powerful, independent tests of our minimax scaling
predictions.
...
limit ($\alpha \approx \VarAlphaWTwo$). This mapping \chA{supports the interpretation} that biological
