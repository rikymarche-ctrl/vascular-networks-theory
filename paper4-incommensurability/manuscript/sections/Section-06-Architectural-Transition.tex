...
physically constrained derivation, the wave penalty
...
\begin{enumerate}
    \item
As $M\to 0$, $\alpha^*(M) \to \alpha_t \approx \VarAlphaT$ (viscous
dominance).
    \item
...
\VarAlphaT$. For $M\to\infty$, the inertial geometric matching condition
...
physical matching condition---combining Euclidean geometry, classical
...
the Euclidean space-filling geometry, the criterion provides the physical
...
wave} (governed by the characteristic admittance $Y_c$) which governs
network reflections. Because $Y_c \propto \sqrt{i\omega\rho\,Y_L}$,
...
Section S1). We emphasise that the isotropic-embedding criterion
...
The intuition behind the two thresholds is that a bifurcation must cope
...
connecting the phenomenological observations with the physical theory.

For clarity, the \emph{quantitative} predictions of this work---the
...
The predictive power of this Phase Decoupling is supported by its global
...
($\mathcal{Q}^{-1}_{Y_c} \ge 1$ for all $\mathrm{Wo} > 0$). Here the
...
\approx 2.0$), supporting the interpretation that the branching geometry
...
($\beta \approx \VarBetaHumanObs$) without fitting the target exponent. The simplified model predicts
...
\caption{\textbf{The Incommensurability Jump: Allometric Transition
    ...
    point with no target-exponent fit. Horizontal dashed lines mark
    the two attractors: the transport limit
    $\alpha_t \approx \VarAlphaTFig$ ($\eta \to 0$) and the
    wave limit $\alpha_w = \VarAlphaW$ ($\eta \to 1$; this is the
...
future work. We stress that the falsifiable predictions below are anchored
...
\begin{table}[H]
\centering
\caption{Predictive verification of the Womersley-driven transition under cardiac frequency override. Throughout, $\mathrm{Wo}_0$ denotes the Womersley number at the aortic root (generation~0, the largest vessel), consistent with the per-generation hierarchy of the Supplemental Material; the mouse and hummingbird entries follow from the mouse baseline via Eq.~\eqref{eq:wo_scaling}.}
...
Human               & 70\,kg & \VarHeartRateHuman & 17.9 & $\VarAlphaStar$ (minimax) \\
\bottomrule
\end{tabular}
\end{table}
...
scaling of Eq.~\eqref{eq:wo_scaling}. \textbf{At present:} To our
...
limits $\alpha_w \approx 2.0$ and $\alpha_t \approx \VarAlphaT$ emerge in
...
incommensurability principle. An expanded validation across 27 distinct
...
The breadth of this validation set is itself the central empirical test of
...
pre-specified, cross-kingdom sorting rule that the 27 systems test
...
work)
    \item
\textbf{Jiang, Kassab \& Fung (1994)}: Rat pulmonary ($M=300\,\mathrm{g}$),
$\alpha_{\mathrm{obs}} = 2.60$--$2.75$ vs.\ $\alpha^*_{\mathrm{pred}} = 2.68$
    \item
\textbf{Huang (1996)}: Human pulmonary ($M=70\,\mathrm{kg}$),
$\alpha_{\mathrm{obs}} = 2.60$--$2.85$ vs.\ $\alpha^*_{\mathrm{pred}} =
\VarAlphaStarElastic$
\end{enumerate}

All three datasets exhibit quantitative agreement with theoretical
...
the 27 systems in the extended validation set (Supplemental Material,
...
its threshold in planar geometries, placing planar networks (retinal
...
($\alpha \approx 2.0$). Additional support is provided by the human retina
...
architecture (diameters) is biased toward the wave regime to minimize
...
area-preserving limit. This "structural tension" is one direct empirical
signature of the Scaling Conflict Bound: the incommensurability between
...
limit ($\alpha \approx \VarAlphaWTwo$). This mapping supports the
