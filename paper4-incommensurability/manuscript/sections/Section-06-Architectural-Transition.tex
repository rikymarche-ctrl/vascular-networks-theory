...
\begin{enumerate}
    \item
As $M\to 0$, $\alpha^*(M) \to \alpha_t \approx 3.0$ (viscous dominance).
    \item
...
3.0$. For $M\to\infty$, the inertial geometric matching condition $\alpha_w
...
($\mathcal{Q}^{-1}_{Y_c} \ge 1$ for all $\mathrm{Wo} > 0$). This geometric
constraint forces planar retinal diameters to permanently transition toward the
wave-attractor limit ($\alpha \approx 2.0$), directly confirming that the
...
\caption{\textbf{The Incommensurability Jump: Allometric Transition
    ...
    wave limit $\alpha_w = \VarAlphaW$ ($\eta \to 1$).
...
future work. \textbf{The Hummingbird Test: A Falsifiable Prediction.} A critical
...
\begin{table}[H]
\centering
\caption{Predictive verification of the Womersley-driven transition under cardiac frequency override.}
...
Human          & \VarMassHuman & \VarHeartRateHuman  & 6.8  & \VarAlphaStar (minimax) \\
\midrule
...
limits $\alpha_w \approx 2.0$ and $\alpha_t \approx 3.0$ emerge in
...
incommensurability principle. An expanded validation across 30 distinct
biological and synthetic networks is provided in the Supplemental Material
(Table S2), showing that the $\alpha^* \approx \VarAlphaStar$ attractor appears
...
the 30+ systems in Table~\ref{tab:multivalid} exhibit wide observed ranges that
overlap with multiple theoretical attractors. This data limitation is rapidly
being resolved by next-generation whole-organ imaging databases: for instance,
complete 3D microCT reconstructions of the mouse cerebral
