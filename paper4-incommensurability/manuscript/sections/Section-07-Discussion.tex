...
\chA{motivates} the minimax framework. We then explore how this theoretical
foundation resolves long-standing paradoxes in vascular biology and helps
explain the architectural divergence across species.
...
but a \chA{structural rationale motivated by} the three theorems established in
this work. Understanding this \chA{rationale} requires connecting the
mathematical results to their biological implementation.
...
\textbf{Step 2: From Theorem~\ref{thm:gauge} (Gauge Invariance).} The
dimensionless linear-excess network-level penalty functional,
...
cardiac duty cycle $\eta_d$)}, yielding the network Lagrangian of
Eq.~\eqref{eq:lagrangian_net}. At this stage, $\eta$ is a
\emph{\chA{undetermined cost-mixing coordinate}} whose value is unspecified by
the theory.

\textbf{Step 3: From Theorem~\ref{thm:arch} (Architectural Invariance).} The
\chA{marginal-balance weight} $\eta^*$ is an exact architectural invariant,
independent of body mass $M$ and metabolic scaling exponents. This invariance is
possible \emph{only if} $\eta$ is \emph{determined by} rather than \emph{input
to} the optimization. If $\eta$ were fixed exogenously and $\alpha$ optimized
for that particular value, otherwise changes in $\eta$ (due to heart rate,
allometric scale, or physiological load) would induce corresponding changes in
$\alpha^*(\eta)$, violating the observed morphometric stability. Instead, the
minimax structure simultaneously determines both the optimal morphology
$(\alpha^*, \beta^*)$ and the emergent \chA{marginal-balance weight} $\eta^*$
such that marginal penalties balance. \chA{Here the cardiac duty cycle $\eta_d =
t_\mathrm{systole}/T_\mathrm{cycle} \in [0.30, 0.50]$ is the physiological
quantity whose envelope motivates the wave/transport weighting, and the minimax
$\alpha^*$ is robust to its variation; the emergent weight $\eta^* \approx
\VarEtaStar$ of Eq.~\eqref{eq:eta_star} is a \emph{derived} diagnostic---the
weight at which the marginal costs balance at the saddle---and is therefore not
constrained to the duty-cycle range.}
...
physiological states \chA{(}from $\chA{\eta_d} \approx 0.30$ at rest to
$\chA{\eta_d} \approx 0.50$ during exercise\chA{)}, differs systematically
across species (human resting HR $\sim$80~bpm vs.~macaque $\sim$200~bpm), and
changes ontogenetically as body mass increases by $10^4$-fold from embryo to
adult.

\chA{Yet} the parameter subject to optimization ($\alpha$) exhibits \emph{two
orders of magnitude less variability} than the physiological constraints it must
accommodate ($\chA{\eta_d}$, heart rate, body mass). This is precisely the
signature of \emph{worst-case} or \emph{minimax} optimization in robust control
theory: the system selects a strategy that performs well across all scenarios
rather than performing optimally in a single scenario at the cost of fragility
elsewhere.
...
heart rate, and cardiac duty cycle, for example $M \in [0.05,100]\,\text{kg}$,
$f_\mathrm{HR}\in[60,200]\,\text{bpm}$, and
...
ranges over $[0,1]$.} We now prove that the equal-cost condition
$\mathcal{C}_{\mathrm{wave}} = \mathcal{C}_{\mathrm{transport}}$ is the
\emph{unique} minimax saddle point of this problem.
...
\end{equation}
Then the unique solution to \chA{$\alpha^* = \arg\min_\alpha \max_{\eta \in
[0,1]} \mathcal{L}(\alpha, \eta)$} satisfies
$\mathcal{C}_{\mathrm{wave}}(\alpha^*) =
\mathcal{C}_{\mathrm{transport}}(\alpha^*)$.
\end{proposition}
\begin{proof}
\chA{Since $\mathcal{L}$ is linear in $\eta$ and $\eta \in [0,1]$, the
adversary's maximum is attained at one of the two endpoints:}
\begin{equation}
...
the vascular network is \chA{\emph{robust} to physiological fluctuations
represented by} the cardiac duty cycle $\chA{\eta_d}$. Any departure $\alpha
\neq \alpha^*$ creates an asymmetry that the adversary (physiological variation)
can exploit by shifting $\eta$ toward the dominant cost channel. Alternatively,
this can be viewed as \textbf{Pareto optimality} between incommensurable
regimes: any $\alpha \neq \alpha^*$ sacrifices wave performance ($\alpha <
\alpha^*$) or transport efficiency ($\alpha > \alpha^*$) without improving the
other, \chA{identifying} the equal-cost equilibrium as the unique non-dominated
solution.
...
transition zone ($\mathrm{Wo} \sim \sqrt{3}$), to $\alpha \to 3.0$ in the
viscous limit (arterioles)} (Section~\ref{sec:womersley_minimax})---the pooled
...
trees. \chA{A diameter-weighted average spanning $\alpha_w \approx \VarAlphaW$
to $\alpha \to 3.0$ can fall well below the transition value $\VarAlphaStar$;
the pooled $2.39$ specifically requires a substantial contribution from the
low-$\alpha$, wave-dominated proximal conduits, and cannot arise from a mixture
confined to the $[\VarAlphaStar, 3.0]$ range alone.}
...
boundary ($\mathrm{Wo} \le \sqrt{3} \approx 1.73$). \chA{The meta-analytic
pooled value of $2.39$ falls within the full $[\alpha_w, 3.0]$ span set by the
wave and viscous attractors---necessarily \emph{below} the transition value
$\VarAlphaStar$, which is consistent only with diameter-weighted sampling that
draws substantially on the low-$\alpha$ proximal conduits.} We acknowledge that
...
\chA{This resolution is consistent with the regime-mixing picture (Remark
below): pooling vessels across Womersley regimes with realistic diameter
distributions can reproduce both a sub-transition pooled mean
($\alpha_{\mathrm{pooled}} \approx 2.4$) and the high between-study
heterogeneity ($I^2 = 99\%$). Rather than contradicting the minimax theory, the
Taylor meta-analysis is therefore compatible with it---though a definitive test
requires Womersley-stratified raw data, because the diameter-weighted pooled
exponent is not directly comparable to the local area-preserving attractor
$\VarAlphaStar$.}

\paragraph{Co-Evolutionary Interpretation.}

The minimax saddle point represents the evolutionary equilibrium of
\emph{heart-vessel co-optimization}. The heart determines the cardiac duty cycle
$\chA{\eta_d}$ through its contractile dynamics and pacing frequency; the
vasculature determines the branching morphology $(\alpha, \beta)$ through
\chE{angiogenesis and remodeling}. Neither subsystem can unilaterally improve
performance without coordinated adjustment of the other. The saddle-point
condition is the formal statement of this mutual optimality: marginal changes in
vascular geometry or cardiac timing are equally costly, indicating that the
system has reached a configuration where further adaptation by either component
alone is disadvantageous.
...
stress and impedance equilibria, \chA{supporting the interpretation} that the
minimax attractor acts as a dynamic homeostatic target during ontogeny. The
\chA{marginal-balance weight} $\eta^*$ is not a physiological input but an
\emph{architectural invariant} emerging from the dimensionless ratios in the
network Lagrangian.
...
Theorem~\ref{thm:gauge} identifies a \chA{scale-invariant escape} from this
constraint: lifting the optimization to the network level, where both cost
functions become dimensionless fractional excesses. \chA{Under the stated
assumptions}, this is not merely a modelling convenience; it is the
\chA{normalization} consistent with scale invariance and thermodynamic
linearity. The analogy with gauge theories in physics is \chA{heuristic, not
formal---a global scaling symmetry rather than the local gauge symmetry of field
theory (cf.\ Remark~\ref{rem:gauge_terminology}):} just as electrodynamics is
\chA{constrained} to be scale-invariant by the requirement that physical
observables be independent of the choice of potential, biological transport
optimization is \chA{constrained} to be metabolically scale-invariant by the
requirement that architectural decisions be independent of absolute
physiological scale.
...
eigenvalue of the allometric class --- \chA{robust} to the fluctuations of body
mass, metabolic rate, and haemodynamic load that characterize ontogenesis. This
...
irrelevant variables, and universality classes is used heuristically to organize
the model's scale behaviour; we do not construct a formal RG transformation for
vascular networks, and the parallel should be read as a guide to intuition
rather than as a theorem. The finite-size correction $\alpha^*(G)$ below is, by
contrast, a genuine consequence of the saddle-point curvature and is stated as a
...
$(\alpha^*, \eta^*)$ is the \chA{configuration predicted to prevent either
constraint} \chA{from becoming a viability bottleneck}. This explains why the
same geometry persists across vastly different metabolic scales: it \chA{acts as
a "least-worst"} compromise between physical laws that cannot be simplified into
one another. In this sense, the minimax is the architectural signature of an
organism evolved for optimal robustness rather than narrow efficiency.
\end{remark}
...
The theoretical framework \chA{reduces reliance} on phenomenological fitting.
The derivation of the critical Womersley number $\mathrm{Wo}_c \approx \VarWoC$
from evanescent mode absorption at the bifurcation junction---combining the
Navier-Stokes viscous absorption capacity ($\mathcal{Q}^{-1} = 6/\mathrm{Wo}^2$)
with the geometric scattering ratio ($E_\perp/E_\parallel = d-1$)---provides a
physically derived threshold for the allometric transition. Furthermore, the
empirical \chA{support} across independent vascular networks---from porcine
coronaries to human retinal vessels and the bronchial tree---\chA{supports the
interpretation} that the dual-attractor minimax is a broad organizing principle
\chA{rather than a dataset-specific artifact}.
...
the minimax is a \chA{mathematically consistent attractor under the stated
assumptions}. Whether this conclusion extends to other classes of
incommensurable biological costs---such as information capacity versus
structural investment in neural networks---requires case-specific verification
that the coupling parameter $\mu$ exhibits the requisite scale-dependence across
the relevant hierarchy, and constitutes an open problem beyond the scope of this
work.
...
efficiency or developmental simplicity) typically favor binary branching
($N=2$), the incommensurability principle ensures that the branching geometry
remains a universal constant regardless of the specific local topology.
\end{remark}
...
transfer-matrix analyses of coupled reflections \chA{indicate} that the minimax
shift remains negligible ($|\Delta\alpha^*| < 0.01$). The incoherent model
effectively isolates the topological branching penalty from local interference
patterns, providing a robust structural predictor of network-wide morphometry.
\end{remark}
...
(\chA{a per-generation radius reduction of $\approx \VarHeteroTaperPercent\%$})
shifts these attractors by approximately \VarAlphaHeteroCombinedShift{}
(yielding \VarAlphaHeteroCombined{} for rigid) or
\VarAlphaHeteroCombinedElasticShift{} (yielding \VarAlphaHeteroCombinedElastic{}
for elastic). Tapering acts as a vital stabilizing mechanism that keeps the
network close to the wave-optimum.
    \item
...
\approx 1000\,$bpm) \chA{places it at the fluid--wave transition ($\mathrm{Wo}_0
\approx 1.76$, just above the threshold $\sqrt{3}$), predicting $\alpha^*
\approx 2.6$ (Table~\ref{tab:hummingbird}), in the transition range and
comparable to large mammals} despite the 10,000-fold mass difference,
\chA{supporting} the Womersley number---not mass---as the \chA{architectural
control parameter}. For typical mammals with allometric heart-rate scaling ($f_H
\propto M^{-1/4}$), the predicted trend is $\alpha^* \approx
\VarAlphaHeteroBase$ near the transition mass $M^* \approx \VarMStar\,$g, rising
to $\alpha^* \approx \VarAlphaStar$--$2.75$ in large mammals as morphometric
heterogeneities accumulate. Meta-analysis of coronary morphometry across the
mammalian mass range (mouse $25\,$g to elephant $4000\,$kg) can test this
Womersley-driven prediction.
...
that morphometric studies typically sample at $\alpha \approx 2.7$)} contain
over $70\text{--}80\%$ of the total blood volume, the global scaling of the
total vascular volume—and thus the metabolic scaling $b$ of the entire
organism—is mathematically dominated by the wave attractor limit ($\alpha \to
2.0$), yielding the classical $b = 3/4$ Kleiber's Law.
...
\chA{physical consequence of the organ-level boundary condition}. The kidney
functions not merely as a transport network, but as a filtration system that
must intentionally impose a massive, localized pressure drop to drive glomerular
filtration:
\begin{equation}
...
outputs, \chA{distinguishing independently constrained inputs from derived
outputs}.
...
values were adjusted to fit $\alpha^* = 2.72$. The decomposition
$\chA{\VarAlphaStarModel} \to 2.72$ attributes each shift to a specific measured
physical effect (elasticity, asymmetry, taper), not \chA{post-hoc fitting
degrees of freedom}.
...
coronary dataset (e.g., Guo 2003 \cite{Guo2003}), which is the same tissue type
used as a primary validation benchmark for $\alpha^*$. This creates a potential
...
\begin{enumerate}
\item
\textbf{Hummingbird coronaries:} \chA{$M \approx 4\,$g $\Rightarrow$ $\alpha
\approx 2.6$ (transition; currently no data).}
\item
...
2000).
\end{enumerate}
\chA{These predictions are fixed once the listed inputs are specified---there is
no} \chA{post-hoc target-exponent fit}.
...
channel-distinguishability criterion of Remark~\ref{rem:incommensurability}. The
purpose here is to collect them in one place and to \chA{specify} what the
framework does \emph{not} establish.}

\paragraph{\chRB{(A1) Optimality.}}
\chRB{We treat the observed morphometry as the solution of an optimization
problem---i.e.\ we assume the vasculature sits at or near a selected optimum.
The principal alternatives are (i)~\emph{developmental canalization}, in which
the branching exponent is a hard-coded morphogenetic program rather than an
emergent optimum; (ii)~\emph{physical self-organization}, in which the exponent
is fixed by the mechanics of angiogenic growth and shear-driven remodeling
independent of any global fitness criterion; and (iii)~\emph{neutral evolution},
in which drift and phylogenetic inertia, rather than selection, set the pattern.
The framework is deliberately agnostic about \emph{mechanism}: it predicts the
...
supplies the \emph{target} (cf.\ the phase-lag hypothesis and the relationship
to dynamical network-formation models discussed above). Alternative~(i), the
genetic-blueprint hypothesis, is the one the framework genuinely contradicts,
and it is the explicit falsification target of the ontogenetic transition test
...
\chRB{We assume selection minimizes the \emph{worst-case} fractional excess
across the physiological envelope (Proposition~\ref{prop:minimax_saddle}), not
the expected cost. The natural alternatives are (i)~\emph{expected-cost
(Bayesian) optimization} over a prior on operating states, which would make
$\alpha^*$ depend on that prior---and hence on species-specific activity
statistics and ecology; (ii)~\emph{single-objective optimization} at a fixed
operating point, which predicts $\alpha^*$ co-varying with heart rate and duty
cycle $\eta_d$ (contradicted by the observed invariance, as argued under
``Contrast with Single-Objective Optimization'' above); and
(iii)~\emph{satisficing}, which predicts broad, unstructured dispersion rather
than a sharp attractor. The empirical signature---$\alpha$ varying by roughly
two orders of magnitude less than the physiological quantities it must
accommodate---favours the minimax and excludes (ii) and (iii). We emphasize,
however, an honest non-uniqueness: a Bayesian optimum~(i) computed against a
broad, roughly symmetric prior over the same envelope converges to nearly the
same $\alpha^*$. Minimax and average-case optimization are therefore close in
their predictions and difficult to separate with present data; we adopt the
minimax as the more conservative, assumption-light choice, because it requires
no commitment to a particular activity prior.}

\paragraph{\chRB{(A3) The informational (epistemic) bound.}}
\chRB{Proposition~\ref{thm:nogo} assumes that a local controller cannot access
the non-local invariants required for single-junction optimization to preserve
...
(ii)~\emph{genetic pre-specification} that bypasses real-time sensing
altogether. Alternative~(i) is constrained by the phase-lag blind spot
established in Section~2: the specific global quantity at issue---the phase of
waves reflected from the network periphery---is invisible on the $\sim$10~ms
timescale of cellular mechanotransduction, so the bound is an argument about
\emph{information availability} that is robust to the choice of signalling
modality. Alternative~(ii) again coincides with the genetic-blueprint hypothesis
and is addressed by the same ontogenetic test.}
...
\chRB{Of these five assumptions, (A1) and (A3) are sharply falsifiable through
the ontogenetic transition test; (A5) is supported by the existing
dual-attractor morphometry and would be overturned only by a single-currency
theory reproducing the same transition; (A4) is largely shielded by parameter
independence but delimits the framework's domain, since multi-currency organs
require an extended Lagrangian; and (A2) is the least separable from its leading
alternative, because worst-case and broad-prior average-case optimization
predict nearly the same attractor. We regard the explicit demarcation of what
the theory assumes, what it predicts, and what it cannot currently distinguish
as a strength rather than a weakness: it identifies precisely which future
measurements---generation- and Womersley-resolved morphometry across ontogeny
and across species with divergent activity statistics---would adjudicate among
