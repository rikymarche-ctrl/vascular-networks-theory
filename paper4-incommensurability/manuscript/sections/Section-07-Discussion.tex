...
where the cost-weight bounds $\eta_{\min} \approx 0.30$ (rest) and $\eta_{\max}
\approx 0.50$ (exercise) are \chA{set by the cardiac duty cycle $\eta_d$ spanning its
full physiological range}. We now prove that
...
the vascular network is \emph{immune} to fluctuations in the cardiac duty cycle $\chA{\eta_d}$.
...
despite the 10,000-fold mass difference, confirming that the Womersley number---not
...
place and to state plainly what the framework does \emph{not} establish.}
