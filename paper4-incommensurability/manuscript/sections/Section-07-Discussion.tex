...
motivates the minimax framework. We then explore how this theoretical
...
\subsection{Why Minimax? Theoretical Rationale and Biological Interpretation}

The minimax formulation of Paper~II is not an arbitrary mathematical framework
but a structural rationale motivated by the three theorems established in
this work. Understanding this rationale requires connecting the
mathematical results to their biological implementation.

\paragraph{Three-Step Logical Rationale.}
...
and points to a network-level framework.
...
introduces the cost-mixing weight $\eta$ (physiologically motivated by the
cardiac duty cycle $\eta_d$), yielding the network Lagrangian of
Eq.~\eqref{eq:lagrangian_net}. At this stage, $\eta$ is a
\emph{undetermined cost-mixing coordinate} whose value is unspecified by
the theory.

\textbf{Step 3: From Theorem~\ref{thm:arch} (Architectural Invariance).} The
marginal-balance weight $\eta^*$ is an exact architectural invariant,
...
$(\alpha^*, \beta^*)$ and the emergent marginal-balance weight $\eta^*$
such that marginal penalties balance. Here the cardiac duty cycle $\eta_d =
...
structural dispersion. The cardiac duty cycle $\eta_d =
t_\mathrm{systole}/T_\mathrm{cycle}$ varies by more than 40\% across
physiological states (from $\eta_d \approx 0.30$ at rest to
$\eta_d \approx 0.50$ during exercise), differs systematically
...
Yet the parameter subject to optimization ($\alpha$) exhibits \emph{two
orders of magnitude less variability} than the physiological constraints it must
accommodate ($\eta_d$, heart rate, body mass). This is precisely the
...
networks must accommodate. The physiological envelope includes body mass,
...
\end{proposition}
\begin{proof}
Since $\mathcal{L}$ is linear in $\eta$ and $\eta \in [0,1]$, the
adversary's maximum is attained at one of the two endpoints:
\begin{equation}
\max_{\eta \in [0,1]} \mathcal{L}(\alpha, \eta) =
\begin{cases}
\mathcal{C}_{\mathrm{wave}}(\alpha)
...
\end{cases}
\end{equation}
Thus the adversarial objective is the upper envelope
...
\begin{equation}
\alpha^* = \arg\min_\alpha \max_{\eta \in [0,1]} \mathcal{L}(\alpha, \eta)
...
the vascular network is \emph{robust} to physiological fluctuations
represented by the cardiac duty cycle $\eta_d$. Any departure $\alpha
...
other, identifying the equal-cost equilibrium as the unique non-dominated
solution.
...
on the Womersley number---transitioning from the wave attractor $\alpha_w
...
trees. A diameter-weighted average spanning $\alpha_w \approx \VarAlphaW$
...
boundary ($\mathrm{Wo} \le \sqrt{3} \approx 1.73$). The meta-analytic
...
This resolution is consistent with the regime-mixing picture (Remark
...
$\eta_d$ through its contractile dynamics and pacing frequency; the
vasculature determines the branching morphology $(\alpha, \beta)$ through
angiogenesis and remodeling. Neither subsystem can unilaterally improve
...
stress and impedance equilibria, supporting the interpretation that the
minimax attractor acts as a dynamic homeostatic target during ontogeny. The
marginal-balance weight $\eta^*$ is not a physiological input but an
...
\begin{remark}[Statistical Dispersion as a Structural Signature]
...
Theorem~\ref{thm:gauge} identifies a scale-invariant escape from this
constraint: lifting the optimization to the network level, where both cost
functions become dimensionless fractional excesses. Under the stated
assumptions, this is not merely a modelling convenience; it is the
normalization consistent with scale invariance and thermodynamic
linearity. The analogy with gauge theories in physics is heuristic, not
...
constrained to be scale-invariant by the requirement that physical
observables be independent of the choice of potential, biological transport
optimization is constrained to be metabolically scale-invariant by the
...
eigenvalue of the allometric class --- robust to the fluctuations of body
...
This subsection develops an \emph{interpretive} analogy with the
...
$(\alpha^*, \eta^*)$ is the configuration predicted to prevent either
constraint from becoming a viability bottleneck. This explains why the
same geometry persists across vastly different metabolic scales: it acts as
...
The framework also accounts for the topological bifurcations observed when
...
expected consequence of the embedding space. The incommensurability
principle thus provides a unified explanation for the "geometric crisis"
across different dimensional and metabolic scales.

The theoretical framework reduces reliance on phenomenological fitting.
...
empirical support across independent vascular networks---from porcine
coronaries to human retinal vessels and the bronchial tree---supports the
...
the minimax is a mathematically consistent attractor under the stated
...
and the transport-metabolic attractor ($\alpha_t \approx \VarAlphaT$) are
independent of the number of daughter vessels, and the $\ln N$ terms in the cost
gradients cancel out identically in the marginal-balance ratio
...
transfer-matrix analyses of coupled reflections indicate that the minimax
...
(a per-generation radius reduction of $\approx \VarHeteroTaperPercent\%$)
...
\approx 1000\,$bpm) places it at the fluid--wave transition ($\mathrm{Wo}_0
...
supporting the Womersley number---not mass---as the architectural
...
Because the large, proximal conduit vessels (the aorta and great arteries,
...
physical consequence of the organ-level boundary condition. The kidney
...
outputs, distinguishing independently constrained inputs from derived
outputs.

\begin{table}[H]
\centering
\caption{Parameter Accounting: independently constrained inputs and derived outputs}
...
$p$ (wall thickness) & 0.77 & \cite{kassab1993} & Fitted to data \\
$\alpha_w$ (wave attractor) & 2.0--2.115 & \cite{paperII} & Theory \\
\midrule
\multicolumn{4}{l}{\textit{Structural heterogeneity (measured)}} \\
$a$ (asymmetry ratio) & \VarHeteroAsymmetryA & \cite{kassab1993} & Morphometry \\
Taper & 8\% & \cite{kassab1993} & Morphometry \\
\midrule
\multicolumn{4}{l}{\textit{Material properties (literature)}} \\
$E_{\mathrm{wall}}$ & 1.5 MPa & Biomech. lit. & Measured \\
$c_{\mathrm{wave}}$ & 5--8 m/s & \cite{huokassab2006} & Literature \\
\midrule
\multicolumn{4}{l}{\textbf{Derived outputs (no target-exponent fit)}} \\
$\alpha^*$ (symmetric) & \VarAlphaStarModel & This work & \textit{Derived} \\
...
$\VarAlphaStarModel \to 2.72$ attributes each shift to a specific measured
physical effect (elasticity, asymmetry, taper), not post-hoc fitting
degrees of freedom.
...
circularity in the symmetric model validation. This is not a benign caveat:
...
\begin{enumerate}
\item
\textbf{Hummingbird coronaries:} $M \approx 4\,$g $\Rightarrow$ $\alpha
...
2000).
\end{enumerate}
These predictions are fixed once the listed inputs are specified---there is
no post-hoc target-exponent fit.
...
The deductive chain culminating in the minimax saddle
...
purpose here is to collect them in one place and to specify what the
framework does \emph{not} establish.

\paragraph{(A1) Optimality.}
We treat the observed morphometry as the solution of an optimization
...
\paragraph{(A2) Minimax rather than average-case objective.}
We assume selection minimizes the \emph{worst-case} fractional excess
...
\paragraph{(A3) The informational (epistemic) bound.}
Proposition~\ref{thm:nogo} assumes that a local controller cannot access
...
\paragraph{(A4) Metabolic grounding of the cost currency.}
We assume the fitness-relevant cost is dominated by metabolic energy, with
...
\paragraph{(A5) Incommensurability itself.}
The foundational premise is that the transport and wave-reflection costs
...
Of these five assumptions, (A1) and (A3) are sharply falsifiable through
