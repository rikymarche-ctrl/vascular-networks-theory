...
dimensionality \emph{within a single network}: the vessel \emph{diameters} obey a
two-dimensional, wave-matching law ($\alpha\approx 2.0$), while the bifurcation
\emph{angles} sit far from the planar $90^\circ$ that the same 2D fluid argument
would suggest, hugging instead a mechanical-tension optimum. No single local
optimization criterion can deliver both at once. The incommensurability framework
resolves the tension by recognizing that the two features answer to
\emph{different} channels: the diameters are fixed by the fluid/wave balance,
which is acutely sensitive to the embedding dimension and collapses to the wave
attractor in a plane (below), whereas the branching angles are fixed by a local
mechanical tension equilibrium that is largely blind to that fluidic collapse.
Diameters and angles are therefore \emph{decoupled}---exactly the
mechanical--fluidic decoupling of the section title---and need not report the same
effective dimension.}
...
For $d=2$: $\mathrm{Wo}_c^{\mathrm{fluid}} = \sqrt{6} \approx \VarWoCTwo$
    ...
    \item
\textbf{Wave threshold (characteristic admittance \chA{$Y_c \propto \sqrt{i\omega\rho\,Y_L}$, equivalently $Y_c\propto\sqrt{1-F_{10}}$}):}
...
\begin{itemize}
    \item
\chA{\textbf{Diameters:} $\alpha_{\text{retina}} \approx 2.0$ (area-preserving wave limit).}
    \item
\chA{\textbf{Angles:} the \emph{counterfactual} planar fluid rule would give
$\varphi \approx 90^\circ$, but the mechanical wall-tension channel remains closer
to the three-dimensional Murray equilibrium, predicting
$\varphi \approx \VarAngleRetinalCalcDeg^\circ$.}
\end{itemize}
...
$\VarAngleRetinalOffset / \VarAngleRetinalSD \approx 0.20\sigma$ (whereas the gap
to the planar $90^\circ$ limit is $0.63\sigma$)}; however, given the large biological
variance ($\sigma = \VarAngleRetinalSD^\circ$), the angular data are \chA{consistent
with mechanical--fluidic decoupling but are not decisive by themselves.} The high variance likely reflects
...
falling to roughly half of its large-tube value by a diameter of $\sim
30\,\mu$m \emph{in vitro}.} In the still narrower retinal microvessels this
reduction lowers the effective transport exponent below \chA{the classical Murray
value ($\alpha = 3.0$)} toward $\alpha_{\text{eff}} \approx \VarAlphaFahraeusEff$
