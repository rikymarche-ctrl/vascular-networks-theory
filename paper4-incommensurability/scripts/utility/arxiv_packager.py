...
# Resolve base paths dynamically based on script location
script_dir = os.path.dirname(os.path.abspath(__file__))
base_dir = os.path.abspath(os.path.join(script_dir, '..', '..'))
out_dir = os.path.abspath(os.path.join(base_dir, '..', 'Arxiv', 'paper4'))
...
with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as z:
    ...
    # Updated to match the new PDF name
    supp_pdf_path = os.path.join(base_dir, 'output', 'Supplemental Material - The Incommensurability Principle in Biological Transport.pdf')
    if os.path.exists(supp_pdf_path):
        z.write(supp_pdf_path, 'anc/Supplemental_Material.pdf')
        print(f"[OK] Added supplemental PDF: {supp_pdf_path}")
    else:
        print(f"Warning: Supplemental PDF not found at {supp_pdf_path}!")
